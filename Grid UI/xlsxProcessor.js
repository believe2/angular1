...
(function() {
    ...
    angular.module('xlsx-model', []).directive('xlsxModel', ['$parse','$rootScope', function($parse, $rootScope) {
        return {
            restrict: 'A',
            link: function(scope, el, attrs) {
                var model = $parse(attrs.xlsxModel);
                var modelSetter = model.assign;
                var readOpts = {type: 'binary'};
                    ...
                    if (attrs.multiple) {
                        ...
                        angular.forEach(el[0].files, function(f) {
                            ...
                            reader.onload = function(file) {
                                ...
                                if (f === el[0].files[len]) {
                                    $rootScope.$broadcast('EXCEL_FILE_LOADED', master);
                                }
                            }
                        ...
                        reader.onload = function(file) {
                            ...
                            $rootScope.$broadcast('EXCEL_FILE_LOADED', data);
                        }
